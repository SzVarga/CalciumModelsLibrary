...
# Simulate Model
output <- detSim_glycphos(input_df,
                            sim_params,
                            model_params)
# Plot output
par(mar = c(5,5,2,5))
plot(output$time, output$Ca, col="blue", xlim=c(0, 100), ylim=c(0,15), type="l", xlab="time [s]", ylab="glycphos [nmol/l]")
lines(output$time, output$Prot_act, col="red", type="l")
axis(side = 4)
mtext(side = 4, line = 3, 'calcium [a.u]')
legend("topright", legend=c("calcium", "glycphos"), col=c("blue", "red"), lty=c(1,1))
